 ...
 */
export function useVerifications(challengeId: number | null, options?: { mine?: boolean }) {
  return useQuery({
    queryKey: [VERIFICATIONS_KEY, challengeId, options?.mine ?? false],
    queryFn: () => fetchVerifications(challengeId!, options),
    enabled: !!challengeId,
